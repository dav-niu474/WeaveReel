...
- **登录**：访问需输入口令（环境变量 `AUTH_PASSWORD`，缺省 `weavereel`），校验通过后颁发 30 天 HttpOnly Cookie；未登录时所有 `/api/*` 返回 401，前端显示登录页
- **设置中心**：顶栏 ⚙ 打开，配置模型供应商（网关地址 / API Key / 文本 / 图片 / 视觉模型），保存到 D1 `settings`，支持一键检测连通性
...
- 连线样式统一（上游有产出即点亮）；非法连线直接拒绝并提示可连目标；连线中点 ⊕ 可插入节点
