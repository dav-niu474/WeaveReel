...
- **左侧悬浮 Dock 四分区**（均可拖入画布落点创建，拖出后面板自动收起）：**＋ 新增**（节点类型）/ **🧊 资产库**（项目产生的图片/视频汇集）/ **🧑‍🎤 主体库**（预置 IP 形象，主体一致性参考）/ **🗂 素材库**（已发布作品 + 场景素材 + 场景模板，模板式复用）；底部状态点显示视觉参考通道健康度
- **🖼 作品库**（顶栏「保存」旁）：历史生成持久登记（D1），居中弹窗浏览，勾选可加入画布或**发布到素材库**
