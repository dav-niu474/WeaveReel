...
- **右侧悬浮 Dock 五分区**：**＋ 新增**（节点类型拖入画布落点创建）/ **🧊 资产库**（项目产生的一切图片/视频自动汇集）/ **🖼 作品库**（历史生成持久登记，勾选可加入画布或**发布到素材库**）/ **🧑‍🎤 主体库**（预置 IP 形象，主体一致性参考）/ **🗂 素材库**（已发布作品 + 预置场景素材 + 场景模板，支持拖拽与点击加画布，供模板式复用）；Dock 面板内容均可拖入画布落点创建，底部状态点显示视觉参考通道健康度
