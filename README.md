...
### 画布模式：三类角色节点 + 四种语义连线
- **素材节点**（进成片）：文案 📝（旁白/字幕源）、镜头 🖼（画面原子）、音频 🎵（BGM，暂为模拟）、视频 🎬（模拟）
- **结构节点**（探索产线）：分镜 ▦（原九宫格，虚线描边）—— 把一句文案或一张概念图拆成 N 个镜头候选，格子可「⬆ 提升为镜头」进入成片
- **组装节点**（成片出口）：成片 ✂️（原合成视频）—— 汇集上游素材，点「⬈ 进编辑器剪辑」进入时间线；它不生成、只组装
- 连线即语义（线上有标签，左下角有图例）：**可视化**（文案→镜头，蓝）/ **参考**（镜头→镜头，绿）/ **拆解·提升**（素材→分镜、分镜→镜头，橙）/ **入片**（素材→成片，红）；非法连线直接拒绝并提示可连目标
- 自动布局按流水线角色分列：文案 → 分镜 → 镜头 → 成片
- 选中成片节点只弹能力工具条（进编辑器 / 导出），不再弹生成面板
