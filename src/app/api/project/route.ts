...
export async function GET() {
    ...
    return NextResponse.json({ nodes: d.nodes ?? [], edges: d.edges ?? [], view: r.view ? JSON.parse(r.view) : null });
}
...
export async function PUT(req: Request) {
    ...
    const db = await getDb();
    const values = { data: JSON.stringify({ nodes: body.nodes, edges: body.edges ?? [] }), view: body.view ? JSON.stringify(body.view) : null, updatedAt: Date.now() };
    const rows = await db.select().from(projects).where(eq(projects.id, "default"));
